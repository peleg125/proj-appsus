...
function getEmptyBook(title = "", amount = "", authors = "", currencyCode = "ILS") {
  return { title, listPrice: { amount, currencyCode, isOnSale: false }, authors }
}
...
function getDefaultFilter() {
  return { txt: "", maxPrice: "200", pageCount: "", isOnSale: "" }
}
...
function addGoogleBook(googleBook) {
  let googleBooks = utilService.loadFromStorage(BOOK_KEY)
  const isBookExists = googleBooks.some((book) => book.id === googleBook.id)
  ...
  if (!isBookExists) {
    const bookToAdd = {
      ...
      listPrice: {
        ...
        currencyCode: utilService.randCurrency(),
        isOnSale: googleBook.saleInfo.saleability === "FOR_SALE" ? true : false,
      },
    }
    googleBooks.push(bookToAdd)
    utilService.saveToStorage(BOOK_KEY, googleBooks)
