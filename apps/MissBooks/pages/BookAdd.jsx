...
const API_URL =
  'https://www.googleapis.com/books/v1/volumes?printType=books&q=effective%2520javascript'
...
export function BookAdd() {
    ...
    fetch(url)
      ...
      .then((data) => {
        const bookTitles = data.items
          .filter((item) =>
            item.volumeInfo.title.toLowerCase().startsWith(query.toLowerCase())
          )
  ...
  const handleAddBook = (title) => {
    const googleBook = data.items.find(
